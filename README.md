...
#### 　　 電源アダプタを接続しての外部電源で使用する場合には次の順序で電源の入り切りを行わないとバックアップが正常に行われません。ただし、S4スイッチがROM側になっていれば電源入り切りの順番に左右されません。

#### 　　 1)電源を入れるときにはEMMボードより先にMZ-700本体の電源を入れる。

#### 　　 2)電源を切るときには先にEMMボードの電源を切ってからMZ-700本体の電源を切る。

　なお、X1本体の通常の電源ON/OFFでの正常動作は確認していますが、電源ON/OFFを瞬間的に繰り返すような無理な使い方をするとEMMのバックアップが正常に動作しないことがあります。電源ON/OFFを瞬間的に繰り返すと電源ユニット(電源アダプタ)にもダメージが及ぶこともあるので電源ON/OFFは間をおいて行ってください。
